...
namespace smallpt {
	 ...
	 inline double SchlickReflectance(double n1, 
												double n2, 
												double c) {
	 ...
	 inline const Vector3 IdealSpecularReflect(const Vector3& d, 
														 const Vector3& n) {
		return d - 2.0 * n.Dot(d) * n;
	}

	 inline const Vector3 IdealSpecularTransmit(const Vector3& d, const Vector3& n, double n_out, double n_in, double& pr, unsigned short *state) {
		...
		const Vector3 d_Tr = Normalize(nn * d - nl * (nn * cos_theta + cl::sycl::sqrt(cos2_phi)));
