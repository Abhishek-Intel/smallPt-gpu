...
namespace smallpt {
	...
	static void Render(std::uint32_t nb_samples) noexcept {
		...
		Vector3* dev_Ls;
		  cudaMalloc((void**)&dev_Ls, nb_pixels * sizeof(Vector3));
		  cudaMemset(dev_Ls, 0, nb_pixels * sizeof(Vector3));

		// Kernel execution
		const dim3 nblocks(w / 16u, h / 16u);
		const dim3 nthreads(16u, 16u);
		kernel<<< nblocks, nthreads >>>(dev_spheres, (sizeof(g_spheres) / sizeof(g_spheres)), 
										w, h, dev_Ls, nb_samples);

		// Set up host memory
		Vector3* Ls = (Vector3*)malloc(nb_pixels * sizeof(Vector3));
		// Transfer device -> host
		  cudaMemcpy(Ls, dev_Ls, nb_pixels * sizeof(Vector3), cudaMemcpyDeviceToHost);
